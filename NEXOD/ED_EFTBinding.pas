...
begin
	...
	begin
		...
		begin
			if lcCtrl.VisualChangesHandler.value <> 'POS' then
				zcErr := TMC_Error.create(-1, 'Extra value is wrong.', 0)
			else
			begin
				result := TRUE;
			end;
	...
	// To print a receipt
	if fcReceiptNature.VisualChangesHandler.value <> '' then
	begin
		if fcCustomerReceipt.VisualChangesHandler.value = '' then
		begin
...
begin
	...
	else if (leResp <= cbrNoAnswer) or (leResp = cbrNo) then
	begin
	...
	else if (leResp = cbrError) then
		manageUICallback(ctError, lcErr.message);
end;
...
begin
	if assigned(acCombo) and assigned(acCombo.VisualChangesHandler) then
	begin
...
begin
	if assigned(acCombo) then
	begin
		lsValue := '';
		if assigned(acCombo.VisualChangesHandler) then
			lsValue := acCombo.VisualChangesHandler.value;

		acCombo.Values.beginUpdate;
		try
			acCombo.Values.clear;
			for liIdx := 0 to acTT.count - 1 do
			begin
				lsSt := acTT[liIdx];
		...
		begin
			if (Length(lsValue) > 0) then
				acCombo.VisualChangesHandler.value := lsValue;

			if (Length(lsValue) = 0) and (acCombo.Values.count > 0) then
				acCombo.VisualChangesHandler.value := acCombo.Values[0].value;
...
var
	...
	leResp: TMC_CallbackResult;
begin
	if (Length(acCtrl.VisualChangesHandler.value) > 0) then
	begin
		...
		else if (leResp = ctError) then
			fcHandler.manageUICallback(ctError, lcErr.message);
	end
	else
	begin
