...
type
	...
	public
		constructor create(aiwidth, aiheight: integer; aeAlign: TMC_BindingAlignControl; const asName: string;
			aiTabOrder, aiMinValue, aiMaxValue, aiIncrementValue: integer; acCaption: IMC_BindingCaption; aeTickStyle: TMC_BindingTickStyle;
			aiFactor: integer; acFactorCaption: IMC_BindingCaption);
...
begin
	if acSource.QueryInterface(IMC_BindingPositionableControl, lcSource) = S_OK then
	begin
		inherited;
...
begin
	...
	if liTabOrder < -1 then
		liTabOrder := high(TTabOrder);
	fiTabOrder := liTabOrder;
end;

procedure TMC_BindingControl.DoDispose;
begin
	if assigned(fcVisualChanges) then
	begin
		fcVisualChanges.dispose;
		fcVisualChanges := nil;
	end;
	if assigned(fcControlEvents) then
	begin
...
begin
	if acSource.QueryInterface(IMC_BindingControl, lcSource) = S_OK then
	begin
		inherited;
...
begin
	fbChanged := abChanged;
	if (not abChanged) and (QueryInterface(IMC_BindingPanelControl, lcPanel) = S_OK) then
		lcPanel.Controls.Changed := abChanged;
end;
...
begin
	result := fbChanged;
	if (not result) and (QueryInterface(IMC_BindingPanelControl, lcPanel) = S_OK) then
		result := lcPanel.Controls.Changed;
end;
...
begin
	clear;
	if acSource.QueryInterface(IMC_BindingControlsList, lcSource) = S_OK then
	begin
...
begin
	...
	for liIdx := 0 to count - 1 do
	begin
		if sameText(asName, Items[liIdx].Name) then
			result := Items[liIdx]
		else
		begin
			if Items[liIdx].QueryInterface(IMC_BindingPanelControl, lcPanel) = S_OK then
				result := lcPanel.Controls.search(asName)
			else if Items[liIdx].QueryInterface(IMC_BindingPagesContainerControl, lcPages) = S_OK then
				result := lcPages.pages.search(asName)
			else if Items[liIdx].QueryInterface(IMC_BindingPageControl, lcPage) = S_OK then
				result := lcPage.Controls.search(asName);
		end;
...
begin
	if acSource.QueryInterface(IMC_BindingCaption, lcSource) = S_OK then
	begin
		inherited;
...
begin
	if acSource.QueryInterface(IMC_BindingListValue, lcSource) = S_OK then
	begin
		inherited;
...
begin
	clear;
	if acSource.QueryInterface(IMC_BindingListValuesList, lcSource) = S_OK then
	begin
...
begin
	if (assigned(fcValueListParent)) and (assigned(fcValueListParent.VisualChangesHandler)) and
		(fcValueListParent.VisualChangesHandler.QueryInterface(IMC_BindingListVisualHandler, lcListHandler) = S_OK) then
		lcListHandler.UpdateVisualList;
end;
...
begin
	if acSource.QueryInterface(IMC_BindingPanelControl, lcSource) = S_OK then
	begin
		inherited;
		freeControls;
...
begin
	if not assigned(fcControls) then
		fcControls := TMC_BindingControlsList.create;
...
begin
	if acSource.QueryInterface(IMC_BindingGroupControl, lcSource) = S_OK then
	begin
		inherited;
		fsCaption := lcSource.caption;
	end
...
begin
	if acSource.QueryInterface(IMC_BindingCheckControl, lcSource) = S_OK then
	begin
		inherited;
		fsCaption := lcSource.caption;
	end
...
constructor TMC_BindingListValuesControl.create(aiwidth, aiheight: integer; aeAlign: TMC_BindingAlignControl; const asName: string;
	aiTabOrder: integer);
begin
	inherited create(aiwidth, aiheight, aeAlign, asName, aiTabOrder);
...
begin
	if acSource.QueryInterface(IMC_BindingListValuesControl, lcSource) = S_OK then
	begin
		inherited;
		freeValues;
...
begin
	if not assigned(fcValues) then
		fcValues := TMC_BindingListValuesList.create(self);
...
begin
	if acSource.QueryInterface(IMC_BindingComboBoxControl, lcSource) = S_OK then
	begin
		inherited;
		freeCaption;
...
begin
	if acSource.QueryInterface(IMC_BindingRadioControl, lcSource) = S_OK then
	begin
		inherited;
		fsCaption := lcSource.caption;
...
begin
	if acSource.QueryInterface(IMC_BindingSpinControl, lcSource) = S_OK then
	begin
		inherited;
...
begin
	if assigned(fcFactorCaption) then
		fcFactorCaption.dispose;
...
begin
	if acSource.QueryInterface(IMC_BindingTrackBarControl, lcSource) = S_OK then
	begin
		inherited;
...
begin
	if acSource.QueryInterface(IMC_BindingEditControl, lcSource) = S_OK then
	begin
		inherited;
...
begin
	if acSource.QueryInterface(IMC_BindingHandler, lcSource) = S_OK then
	begin
		inherited;
		fsCaption := lcSource.caption;
...
begin
	...
	if (length(asToken) > 0) and (length(asConnectString) > 0) then
	begin
		lsToken := ';' + asToken + '=';
...
begin
	if assigned(fcCallback) then
		fcCallback.onAddLog(aeSeverity, 'ED_EFT', className + asMessage);
end;
...
begin
	...
	if assigned(fcCallback) then
		result := fcCallback.manageLog(aeSeverity);
end;
...
begin
	if doManageLog(lsVerbose) = cbrOk then
		doAddLog(lsVerbose, '(TMC_BindingHandler).manageMC_CallBack entrée : asCommand="' + asCommand + '"');
	...
	try
		try
			if assigned(fcCallback) then
				result := fcCallback.onMC_Callback(nil, dtApplication, -1, TStreamAdapter.create(lcInput, soReference) as IStream, lcAnswer, zcError);
			if result = cbrOk then
			begin
				if assigned(lcAnswer) then
					zsAnswer := wideStringfromStream(lcAnswer);
			end
			else if assigned(zcError) then
				result := cbrError;
		finally
			lcInput.free;
		end;
		...
		begin
			...
			if not assigned(zcError) then
				zcError := TMC_Error.create(1, lsError, 0);
			result := cbrError;
		end;
	end;

	if doManageLog(lsVerbose) = cbrOk then
		doAddLog(lsVerbose, '(TMC_BindingHandler).manageMC_CallBack sortie : result="' + inttostr(ord(result)) + '"');
end;
...
begin
	...
	if assigned(lcCtrl) then
	begin
		if assigned(lcCtrl.VisualChangesHandler) then
			lcCtrl.VisualChangesHandler.value := lsValue;
	end
	else
		manageUICallback(ctError, Format('Control %s not found!', [asParamName]))
end;
...
begin
	if not assigned(fcPages) then
		fcPages := TMC_BindingPagesList.create();
