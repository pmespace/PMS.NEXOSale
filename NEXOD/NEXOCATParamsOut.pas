...
type
	...
	public
		constructor create(const asShopCode, asTillCode: widestring; aeTransactionResult: TMC_CATResultTransaction; const asMessageTransaction: widestring;
			acGiftCard: IMC_CATPOGiftCard); reintroduce; overload;
...
begin
	if acSource.queryInterface(IMC_CATParamsOut, lcSource) = S_OK
	then begin
		inherited;
		...
		fcTransacList := nil;
		if assigned(lcSource.transacList)
		then transacList.assign(lcSource.transacList);
		fcGiftCard := nil;
	end
	else assignError;
end;
...
begin
	if not assigned(fcTransacList)
	then fcTransacList := TNexoCATPOTransacList.create;
...
constructor TNexoCATPOTransac.create(const asResultCode, asapprovedModeTxt, asApprovedTransactionNumber: widestring; afApprovedAmount: double;
	const asApprovedCurrency: widestring; aeApprovedCurKind: TMC_CATCurrencyKindCode; const asCardNumber, asCardExpirationDate, asCardOwnerName,
	asApprovedAuthNum, asApprovedPaymentMode, asCmc7: widestring; aeDetaxeStatus: TMC_CATDetaxeStatus; aeMustSign: TMC_CATMustSign; const asExtraData: widestring;
	aiTermQty: integer; acDCC: IMC_CATPODCC; afGiftCardCreditLeft: double; const asCreditLeftCurrency: widestring; aeCreditLeftCurKind: TMC_CATCurrencyKindCode);
begin
	inherited create('');
...
begin
	if acSource.queryInterface(IMC_CATPOTransac, lcSource) = S_OK
	then begin
		inherited;
...
begin
	clear;
	if acSource.queryInterface(IMC_CATPOTransacList, lcSource) = S_OK
	then begin
		...
		for liIdx := 0 to lcSource.count - 1 do begin
			...
			lcInfo := TNexoCATPOTransac.create(lcItem.resultCode, lcItem.approvedModeTxt, lcItem.approvedTransactionNumber, lcItem.approvedAmount,
				...
				lcItem.approvedPaymentMode, lcItem.CMC7, lcItem.DetaxeStatus, lcItem.MustSign, lcItem.ExtraData, lcItem.termQty, lcItem.DCC, lcItem.GiftCardCreditLeft,
				lcItem.CreditLeftCurrency, lcItem.CreditLeftCurKind);
