...
type
	...
	protected
		...
		procedure setError(acError: IMC_Error);
...
begin
	...
	then result := callback.onMC_UICallback((self as IMC_PluginBase), aeType, acParamsIn, zcParamOut);

	AddLog((fsSendTraceAsUICallback = CEGID_STRING_FALSE) and (doManageLog(lsVerbose) = cbrOk), lsVerbose, CFER + inttostr(result) + ' - zcParams.assigned: '
			+ booltostr_(assigned(zcParamOut)));
end;

function TNexoPluginBase.doMC_Callback(aeDeviceType: TMC_DeviceType; aiDeviceNum: integer; acInput: IStream; out zcAnswer: IStream; out zcErr: IMC_Error)
	: TMC_CallbackResult;
begin
	...
	result := cbrNoAnswer;
	if assigned(callback)
	then result := callback.onMC_Callback((self as IMC_PluginBase), aeDeviceType, aiDeviceNum, acInput, zcAnswer, zcErr);
...
begin
	if fsSendTraceAsUICallback = CEGID_STRING_TRUE
	then doUICallback(ctMessage, TMC_UICallbackPIMessage.create('- TRACE - severity: ' + inttostr(ord(aeSeverity)) + '- ' + DRIVER_NAME_SHORT + ' - class: '
					+ className + ' - ' + asMessage, 0), lcParamOut)
	else
		if assigned(callback)
		then callback.onAddLog(aeSeverity, GetLogClass, 'class: ' + className + ' - ' + asMessage);
end;
...
begin
	...
	if pos(';' + fsSendTraceAsUICallback + ';', ';X;-;?;') = 0
	then fsSendTraceAsUICallback := '?';

	if fsSendTraceAsUICallback = '?'
	then fsSendTraceAsUICallback :=
			booltostr_(cbrOk = doUICallback(ctAsk, TMC_UICallbackPIMessage.create('Do you want to see ALL data sent to trace tools (log tool) as UICalback?', 0),
				lcParamOut));
	...
	then result := cbrOk
		...
		if assigned(callback)
		then result := callback.manageLog(aeSeverity);
end;
...
procedure TNexoPluginBase.setError(acError: IMC_Error);
begin
	fcError := acError;
	if assigned(fcError)
	then begin
		_F('TNexoPluginBase.setError');
		AddLogError(CF + ' - Error code: ' + inttostr(fcError.code) + ' - message: ' + fcError.message + ' - extraInfo: ' + inttostr(fcError.extraInfo));
	end;
end;
...
begin
	...
	if assigned(fcError)
	then begin
		if fcError.QueryInterface(IMC_DetailedError, lcDetailedError) = S_OK
		then result := TMC_DetailedError.create(lcDetailedError.code, lcDetailedError.message, lcDetailedError.extraInfo, lcDetailedError.TechnicalMessage,
				lcDetailedError.AdviceMessage, lcDetailedError.Critical)
		else result := TMC_Error.create(fcError.code, fcError.message, fcError.extraInfo);
	end;
...
begin
	...
	case aiCapability of
		caCheckHealthBeforeOpen: result := CEGID_STRING_NO;
		caAlwaysOpen: result := CEGID_STRING_FALSE;
		caManagesStandaloneSettings: result := CEGID_STRING_FALSE;
		caMC_CallbackID: result := CEGID_STRING_FALSE;
		else AddLogError(CF + ' - Capability #' + inttostr(aiCapability) + ' out of index');
	end;
...
begin
	...
	if aiCapability > caUnknown
	then result := doGetCapabilities(aiCapability, asOptArgs)
	else setError(TMC_Error.create(28, 'Capability out of index', 0));
...
begin
	...
	then begin
		setError(nil);
		if length(asCommand) > 0
		then result := doDirectCommand(asCommand, acDataIn, zcDataOut)
		else setError(TMC_Error.create(30, 'No command to execute', 0));
	end
	else begin
		lcErr := getLastError;
